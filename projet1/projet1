...
contract Voting is Ownable {
    ...
    function startProposalsRegistration() external onlyOwner isAtStatus(WorkflowStatus.RegisteringVoters) {
        WorkflowStatus previous = workflowStatus;
        workflowStatus = WorkflowStatus.ProposalsRegistrationStarted;
        emit WorkflowStatusChange(previous, workflowStatus);
    }

    function endProposalsRegistration() external onlyOwner isAtStatus(WorkflowStatus.ProposalsRegistrationStarted) {
        WorkflowStatus previous = workflowStatus;
        workflowStatus = WorkflowStatus.ProposalsRegistrationEnded;
        emit WorkflowStatusChange(previous, workflowStatus);
    }

    function startVotingSession() external onlyOwner isAtStatus(WorkflowStatus.ProposalsRegistrationEnded) {
        WorkflowStatus previous = workflowStatus;
        workflowStatus = WorkflowStatus.VotingSessionStarted;
        emit WorkflowStatusChange(previous, workflowStatus);
    }

    function endVotingSession() external onlyOwner isAtStatus(WorkflowStatus.VotingSessionStarted) {
        WorkflowStatus previous = workflowStatus;
        workflowStatus = WorkflowStatus.VotingSessionEnded;
        emit WorkflowStatusChange(previous, workflowStatus);
    }

    function tallyVotes() external onlyOwner isAtStatus(WorkflowStatus.VotingSessionEnded) {
